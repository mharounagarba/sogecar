...
import './ActesTable.css'

export default function ActesTable({
  ...
  const filtered = data.filter(a =>
    ...
    a.typePEC?.toLowerCase().includes(search.toLowerCase())
  )
      ...
      <table className="material-table">
        <thead>
          <tr>
            ...
            <th onClick={() => toggleSort('matricule')}>Matricule {sortArrow('matricule')}</th>
            <th onClick={() => toggleSort('type')}>Type {sortArrow('type')}</th>
            <th onClick={() => toggleSort('typePEC')}>Type PEC {sortArrow('typePEC')}</th>
            <th onClick={() => toggleSort('montant')}>Montant {sortArrow('montant')}</th>
                ...
                <div className="dropdown-wrapper" ref={el => dropdownRefs.current[a.id] = el}>
                  ...
                  {dropdownOpenId === a.id && (
                    <div className={`dropdown-menu ${dropdownDirection === 'top' ? 'top' : ''}`}>
                      <button onClick={() => navigate(`/actes/${a.id}/update`)}><FaPen /> Modifier</button>
