...
import fields from './beneficiaireFields'
...
export default function BeneficiaireForm({ mode }) {
  ...
  useEffect(() => {
    if (isEdit && id) {
      window.api.getBeneficiaireById(parseInt(id)).then(setForm)
    }
  }, [isEdit, id])
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    if (isEdit) await window.api.updateBeneficiaire(form)
    else await window.api.addBeneficiaire(form)
  ...
  return (
    <Layout>
      <form className="standard-form" onSubmit={handleSubmit}>
        <Title name={isEdit ? 'Modification beneficiaire' : 'Nouveau beneficiaire'} />
