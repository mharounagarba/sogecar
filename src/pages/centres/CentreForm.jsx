...
import fields from './centreFields'
...
export default function CentreForm({ mode }) {
  ...
  useEffect(() => {
    if (isEdit && id) {
      window.api.getCentreById(parseInt(id)).then(setForm)
    }
  }, [isEdit, id])
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    if (isEdit) await window.api.updateCentre(form)
  ...
  return (
    <Layout>
      <form className="standard-form" onSubmit={handleSubmit}>
        <Title name={isEdit ? 'Modification centre' : 'Nouveau centre'} />
