...
export default function ClientsTable({
  ...
  const filtered = data.filter(a =>
    ...
    a.typeClient?.toLowerCase().includes(search.toLowerCase())
  )
      ...
      <table className="material-table">
        <thead>
          <tr>
            ...
            <th onClick={() => toggleSort('typeClient')}>Type {sortArrow('typeClient')}</th>
            <th onClick={() => toggleSort('adresse')}>Adresse{sortArrow('adresse')}</th>
            <th onClick={() => toggleSort('contact')}>Contact{sortArrow('contact')}</th>
            <th onClick={() => toggleSort('telephone')}>Téléphone {sortArrow('telephone')}</th>
                ...
                <div className="dropdown-wrapper" ref={el => dropdownRefs.current[a.id] = el}>
                  ...
                  {dropdownOpenId === a.id && (
                    <div className={`dropdown-menu ${dropdownDirection === 'top' ? 'top' : ''}`}>
                      <button onClick={() => navigate(`/clients/${a.id}/update`)}><FaPen /> Modifier</button>
